...
The applied schema is authoritative for the current backend. JPA entity mappings and simple CRUD repositories now cover the seven question-bank tables listed above. The broader tables later in this document (subjects, topics, practice, mock tests, media, and payments) remain design/planning material unless listed above.
